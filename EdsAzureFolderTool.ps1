...
$logFile = ".\FolderCopyLog_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"

# Read project IDs from text file (one ID per line)
$idsFile = ".\FolderIDs.txt"
if (-not (Test-Path $idsFile)) {
    Write-Host "Error: FolderIDs.txt not found in current directory" -ForegroundColor Red
    exit
}
